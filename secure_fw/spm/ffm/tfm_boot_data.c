...
void tfm_core_get_boot_data_handler(uint32_t args[])
{
    ...
    for (; offset < tlv_end; offset += next_tlv_offset) {
        ...
        /* Re-map module IDs */
        if(GET_IAS_CLAIM(tlv_entry.tlv_type) == SW_BOOT_RECORD) {
            if(GET_IAS_MODULE(tlv_entry.tlv_type) == 0)
                tlv_entry.tlv_type = SET_TLV_TYPE(TLV_MAJOR_IAS,
                                        SET_IAS_MINOR(SW_SPE, SW_BOOT_RECORD));
            else if(GET_IAS_MODULE(tlv_entry.tlv_type) == 1)
                tlv_entry.tlv_type = SET_TLV_TYPE(TLV_MAJOR_IAS,
                                        SET_IAS_MINOR(SW_NSPE, SW_BOOT_RECORD));
        }

        /* Copy back into array */
        (void)spm_memcpy((void *)offset, &tlv_entry,
                         SHARED_DATA_ENTRY_HEADER_SIZE);
