                                 ...
                                 struct tfm_conn_handle_t *conn_handle)
{
    ...
    TFM_CORE_ASSERT(service);
    TFM_CORE_ERROR_HANDLE(conn_handle != NULL);
 ...
 */
static uint32_t get_partition_idx(int32_t partition_id)
{
    uint32_t i;
        ...
        for (j = 0; j < tfm_core_irq_signals_count; ++j) {
            if (tfm_core_irq_signals[j].partition_id ==
                ...
                if (partition->p_static->psa_ff_ver == 0x0100) {
                    tfm_spm_hal_enable_irq((IRQn_Type)tfm_core_irq_signals[j].irq_line);
                } else if (partition->p_static->psa_ff_ver == 0x0101) {
                    tfm_spm_hal_disable_irq((IRQn_Type)tfm_core_irq_signals[j].irq_line);
                }
            }
        }
