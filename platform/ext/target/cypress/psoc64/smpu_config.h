 ...
 * will be retrieved at runtime, either from provisioning data
 * or from hard-coded values in flash_layout.h.
 */
...
#define SMPU0_SUBREGION_DIS ((uint8_t)CY_PROT_SUBREGION_DIS5 | \
                             (uint8_t)CY_PROT_SUBREGION_DIS6 | \
                             (uint8_t)CY_PROT_SUBREGION_DIS7)
#define USED_SUBREGIONS 5
 ...
 */
#if FLASH_S_PARTITION_SIZE != (USED_SUBREGIONS * REGIONSIZE_TO_BYTES(SMPU0_REGIONSIZE)/8)
#error "Flash layout has changed - FLASH_S_PARTITION_SIZE isn't the right fraction of SMPU0_REGIONSIZE"
#endif
...
#define SMPU6_BASE         S_RAM_ALIAS(0)
...
#if REGIONSIZE_TO_BYTES(SMPU6_REGIONSIZE) != \
    (S_UNUSED_SIZE + S_BOOT_DATA_SIZE + S_UNPRIV_DATA_SIZE)
#error "SMPU6_REGIONSIZE should match S_UNUSED_SIZE + S_BOOT_DATA_SIZE + S_UNPRIV_DATA_SIZE"
#endif
...
#define SMPU7_SUBREGION_DIS ((uint8_t)CY_PROT_SUBREGION_DIS0 | \
                             (uint8_t)CY_PROT_SUBREGION_DIS1)
