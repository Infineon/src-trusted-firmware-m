...
/**
 * \brief Open the initial attestation key
 *
 * In the case that the private key could not be accessed directly,
 * this function get its key handle instead.
 *
 * \param[out]     handle  Space to store the key handle
 * \param[out]     curve   The type of the EC curve
 *
 * \return Returns error code specified in \ref tfm_plat_err_t
 */
enum tfm_plat_err_t
...
/**
 * \brief Close the initial attestation key
 *
 * \param[in]      handle  Key handle to close
 *
 * \return Returns error code specified in \ref tfm_plat_err_t
 */
enum tfm_plat_err_t
