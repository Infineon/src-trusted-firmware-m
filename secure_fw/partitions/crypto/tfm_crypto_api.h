...
/**
 * \brief Search the local storage by key ID and return the key handle if found
 *
 * \param[in] key_id      vendor key ID
 * \param[in] key_handle  pointer to the key handle
 *
 * \return Return values as described in \ref psa_status_t
 */
psa_status_t tfm_crypto_get_handle_by_id(psa_key_id_t key_id,
