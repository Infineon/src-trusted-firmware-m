...
/*!
 * \struct ps_obj_table_hdr_t
 *
 * \brief Object table header structure.
 */
struct ps_obj_table_hdr_t {
#ifdef PS_ENCRYPTION
...
#define PS_OBJ_TABLE_HDR_SIZE sizeof(struct ps_obj_table_hdr_t)

/* Specifies the maximum possible number of entries in the table.
 * This needs to be greater than or equal to the number of assets plus one
 * extra entry to store a new object when the code processes a change in a file.
 */
#define PS_OBJ_TABLE_ENTRIES ((PS_MAX_OBJECT_DATA_SIZE - PS_OBJ_TABLE_HDR_SIZE) \
                              / sizeof(struct ps_obj_table_entry_t))
 ...
 */
struct ps_obj_table_t {
  struct ps_obj_table_hdr_t hdr; /*!< Header. */
...
PS_UTILS_BOUND_CHECK(OBJ_TABLE_NOT_FIT_IN_STATIC_OBJ_DATA_BUF,
                     PS_OBJ_TABLE_SIZE, PS_MAX_OBJECT_DATA_SIZE);
                                              ...
                                              struct ps_obj_table_t *obj_table)
{
    ...
    union ps_crypto_t *crypto = &obj_table->hdr.crypto;
    psa_status_t err;
    ...
    TFM_COVERITY_DEVIATE_LINE(overrun, "crypto is treated as raw data")
...
{
    struct ps_crypto_assoc_data_t assoc_data;
    union ps_crypto_t *crypto = &init_ctx->p_table[table_idx]->hdr.crypto;
    psa_status_t err;
                                      ...
                                      struct ps_obj_table_init_ctx_t *init_ctx)
{
    ...
    union ps_crypto_t *crypto =
        &init_ctx->p_table[PS_OBJ_TABLE_IDX_0]->hdr.crypto;
    ...
    /* Authenticate table 1 if data is valid */
    if (init_ctx->table_state[PS_OBJ_TABLE_IDX_1] != PS_OBJ_TABLE_INVALID) {
        crypto = &init_ctx->p_table[PS_OBJ_TABLE_IDX_1]->hdr.crypto;
...
#else
    obj_table->hdr.swap_count++;

    if (obj_table->hdr.swap_count == PS_FLASH_DEFAULT_VAL) {
                                      ...
                                      struct ps_obj_table_init_ctx_t *init_ctx)
{
    ...
    if (PS_OBJECT_SYSTEM_VERSION !=
        init_ctx->p_table[PS_OBJ_TABLE_IDX_0]->hdr.version) {
        init_ctx->table_state[PS_OBJ_TABLE_IDX_0] = PS_OBJ_TABLE_INVALID;
    }

    if (PS_OBJECT_SYSTEM_VERSION !=
        init_ctx->p_table[PS_OBJ_TABLE_IDX_1]->hdr.version) {
        init_ctx->table_state[PS_OBJ_TABLE_IDX_1] = PS_OBJ_TABLE_INVALID;
    }
}
                                ...
                                const struct ps_obj_table_init_ctx_t *init_ctx)
{
    ...
    uint8_t table0_swap_count =
                init_ctx->p_table[PS_OBJ_TABLE_IDX_0]->hdr.swap_count;
    uint8_t table1_swap_count =
                init_ctx->p_table[PS_OBJ_TABLE_IDX_1]->hdr.swap_count;
#endif
...
psa_status_t ps_object_table_create(void)
{
    ...
    p_table->hdr.version = PS_OBJECT_SYSTEM_VERSION;
...
#ifdef PS_ENCRYPTION
    ps_crypto_set_iv(&ps_obj_table_ctx.obj_table.hdr.crypto);
#endif
