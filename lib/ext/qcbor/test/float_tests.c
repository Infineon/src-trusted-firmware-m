...
{
    /* IAR has bug in implementation of isnan() function so this test fails
     * TODO: enable test for IAR once the bug is fixed */
#if defined(__ICCARM__)
    #warning HalfPrecisionAgainstRFCCodeTest qcbor test is disabled due to bug in IAR isnan() function
#else
...
static const uint8_t spExpectedSmallest[] = {
    /* For IAR map is 1 entry smaller */
#if !defined(__ICCARM__) || (__SUBNORMAL_FLOATING_POINTS__ == 1)
    0xB4,
#else
    0xB3,
#endif
    0x64, 0x7A, 0x65, 0x72, 0x6F, 0xF9, 0x00, 0x00, 0x6D,
    ...
    0x69, 0x6E, 0x66, 0x69, 0x6E, 0x69, 0x74, 0x69, 0x74, 0x79,
    0xF9, 0xFC, 0x00, 0x63, 0x4E, 0x61, 0x4E, 0xF9,
    /* NAN has different format for IAR */
#if !defined(__ICCARM__)
    0x7E, 0x00,
#else
    0x7F, 0xFF,
    ...
    /* Subnormal floats are not supported by IAR for CM0+ and CM1 */
#if !defined(__ICCARM__) || (__SUBNORMAL_FLOATING_POINTS__ == 1)
...
#define QCBOREncode_AddDoubleAsSmallestToMapN QCBOREncode_AddDoubleToMapN
    ...
    /* Subnormal floats are not supported by IAR for CM0+ and CM1
     * So if IAR is used, __SUBNORMAL_FLOATING_POINTS__ will determine whether
     * subnormal floats are supported for current processor */
#if !defined(__ICCARM__) || (__SUBNORMAL_FLOATING_POINTS__ == 1)
    // 70                                   # text(16)
