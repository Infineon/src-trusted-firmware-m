...
int32_t tfm_mailbox_hal_init(struct secure_mailbox_queue_t *s_queue)
{
    ...
    /* Sanity check of the NPSE mailbox queue address */
    if(((uintptr_t)ns_queue < NS_DATA_START )|| ((uintptr_t)ns_queue > \
       (NS_DATA_LIMIT - sizeof(struct ns_mailbox_queue_t)))) {
        ERROR_MSG("NSPE mailbox queue is outside of NSPE memory.\r\n")
        tfm_core_panic();
    }
