...
/*
 * FLASH0_SECTOR_SIZE is the value reported to ITS/PS by
 * xxx_FLASH_DRIVER.GetInfo()->sector_size;
 *   e.g.: fs_cfg_ps.sector_size & fs_cfg_its.sector_size
 * Cypress uses different concept than master branch to calculate
 * the Object Table entries
 *   PS_OBJ_TABLE_ENTRIES, which uses TFM_HAL_PS_SECTOR_SIZE
 * Since the code is diverted too much, if the values do not match,
 * the whole concept has to be re-designed.
 *
 */
#if (FLASH0_SECTOR_SIZE != FLASH_AREA_IMAGE_SECTOR_SIZE)
#  error "SECTOR_SIZE does not match"
#endif
...
static int32_t ARM_Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
    ...
    /* Using tfm_memcpy for reading */
    tfm_memcpy(data, (void *)start_addr, cnt);
                                     ...
                                     uint32_t cnt)
{
    ...
    /* Check if address and data size are within available flash memory */
    if ((addr >= FLASH0_SIZE) || (cnt > (FLASH0_SIZE - addr))) {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
    ...
    address = FLASH0_DEV->memory_base + addr;
    ...
    while (cnt)
    {
        ...
        TFM_COVERITY_FP_LINE(cert_int30_c, "cnt is checked to fit into available flash memory")
        ...
        tfm_memcpy(prog_buf, (const void *)row_address, CY_FLASH_SIZEOF_ROW);
        tfm_memcpy(prog_buf + offset, data_ptr, chunk_size);

        status = Cy_Flash_ProgramRow(row_address, (const uint32_t *)prog_buf);
...
static int32_t ARM_Flash_EraseSector(uint32_t addr)
{
    ...
    cy_en_flashdrv_status_t status = Cy_Flash_EraseRow(start_addr);
