...
#define NS_CODE_START (memory_regions.non_secure_code_start)
#define NS_CODE_LIMIT (memory_regions.non_secure_partition_limit)
#endif
...
enum tfm_status_e tfm_has_access_to_region(const void *p, size_t s, uint32_t attr)
{
    ...
    if ((attr & TFM_HAL_ACCESS_WRITABLE) && (attr & TFM_HAL_ACCESS_READABLE)) {
        ...
        return (enum tfm_status_e)TFM_HAL_ERROR_INVALID_INPUT;
    }
