...
/* Merge note: Do not define memory_regions structure values.
 * Call populate_memory_regions()::tfm_nspe_flash_block()
 * to get memory_regions information
 */
struct memory_region_limits memory_regions;
struct policy_hw_settings hw_settings;
...
static void populate_memory_regions(void)
{
    uint32_t nspe_base;
    size_t nspe_size;

    tfm_nspe_flash_block(&nspe_base, &nspe_size);
    TFM_COVERITY_DEVIATE_LINE(cert_int30_c, "NSPE address is a constant or taken from a policy that is validated before provisioning")
    memory_regions.non_secure_code_start = nspe_base + CYBL_HEADER_SIZE;
    memory_regions.non_secure_partition_base = nspe_base;
    TFM_COVERITY_DEVIATE_LINE(cert_int30_c, "NSPE address and size are constants or taken from a policy that is validated before provisioning")
    memory_regions.non_secure_partition_limit = nspe_base + nspe_size - 1;
}
