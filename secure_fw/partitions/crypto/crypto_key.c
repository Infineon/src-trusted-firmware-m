                                        ...
                                        psa_key_id_t key_handle)
{
    ...
    if (index >= TFM_CRYPTO_MAX_KEY_HANDLES ) {
                                 ...
                                 size_t out_len)
{
    ...
    if ((client_key_id >= PSA_KEY_ID_VENDOR_MIN) &&
        (client_key_id <= PSA_KEY_ID_VENDOR_MAX)) {
        return tfm_crypto_vendor_key_open_key(encoded_key, key);
    }
                                          ...
                                          size_t out_len)
{
    ...
    /* Redirect only device key.
     * Other public key operations are taken care of by TF-M Crypto Service
     */
    if (tfm_crypto_vendor_key_is_valid_device_key(key)) {
        /* Found VENDOR device key */
        return tfm_crypto_vendor_key_export_public_key(key,
            data, data_size, &(out_vec[0].len));
    }
