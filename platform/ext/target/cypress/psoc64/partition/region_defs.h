...
/* IAR preprocessor inserts space between dot and section name,
 * so we should wrap section name in string.
 */
#define CY_LINKER_SECTION_NAME(x)       #x
...
/* 64kb for SFB and 16kb for SRAM_DAP of RAM (at the end of the SRAM) are
 * reserved for system use. Using this memory region for other purposes
 * will lead to unexpected behavior.*/
/* FixMe: confirm exact available amount of RAM based on the actual
   system allocation */
#define TOTAL_RAM_SIZE (0x000EC000) /* CY_SRAM_SIZE - 80KB */
 ...
 */
/* Merge note: use 0x400 (d8b5237841b1 Fixes for Initial Attestation Service) */
 ...
 *    0x0800_8000 - 0x0802_F7FF Secure privileged data (S_PRIV_DATA_SIZE, 158KB)
 *    0x0802_F800 - 0x0802_FFFF Secure priv code executable from RAM (S_RAM_CODE_SIZE, 2KB)
 *
 * 0x0803_0000 - 0x080E_BFFF Non-secure (752KB)
 *    0x0803_0000 - 0x080E_AFFF Non-secure OS/App (748KB)
 *      0x080E_AC00 - 0x080E_AFFF PSA NVMEM (PSA_NVMEM_SIZE, 1KB) (if PSA_API_TEST_ENABLED)
 *    0x080E_B000 - 0x080E_BFFF Shared memory (NS_DATA_SHARED_SIZE, 4KB)
 * 0x080E_C000 - 0x080F_FFFF System reserved memory (80KB)
...
#define S_RAM_CODE_SEC_NAME  CY_LINKER_SECTION_NAME(.cy_ramfunc)
#endif
...
#define PSA_API_TEST_NVMEM_START    \
        (NS_DATA_START + NS_DATA_SIZE - PSA_API_TEST_NVMEM_SIZE)
/* PSA NVMEM <= NS_DATA_SIZE */
#if (PSA_API_TEST_NVMEM_SIZE) > NS_DATA_SIZE
...
/* Merge note: System reserved memory is used by SysCall */
/* System reserved memory */
#define SYS_RAM_START   (S_RAM_ALIAS(TOTAL_RAM_SIZE))
#define SYS_RAM_SIZE    0x14000
