...
typedef struct {
    ...
    bool     enabled;                  /* Whether UART is enabled or not */
...
static int32_t ARM_USARTx_Initialize(UARTx_Resources* uart_dev)
{
    ...
    if (hw_settings.uart_base == (uint32_t)uart_dev->base) {
...
#else
    (void)uart_dev; /* unused parameter */

    /* UART initialization is done on the cm0p side (if enabled)
     * Otherwise, cm4 side tests can rely on init done by CyBootloader
     */
