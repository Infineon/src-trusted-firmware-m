                                  ...
                                  size_t out_len)
{
    ...
    if (tfm_crypto_vendor_key_is_valid_key(key_id)) {
        /* Re-direct the call to vendor specific implmentation */
        status = tfm_crypto_vendor_key_sign_hash(
            key_id, alg, hash, hash_length,
                                    ...
                                    size_t out_len)
{
    ...
    if (tfm_crypto_vendor_key_is_valid_key(key_id)) {
        /* Re-direct the call to vendor specific implmentation */
        status = tfm_crypto_vendor_key_verify_hash(key_id, alg, hash, hash_length,
        ...
        status = psa_verify_hash(encoded_key, alg, hash, hash_length,
                                 signature, signature_length);
    }
    return status;
#endif /* TFM_CRYPTO_ASYMMETRIC_MODULE_DISABLED */
