...
- ``ITS_MAX_ASSET_SIZE`` - Defines the maximum asset size to be stored in the
  ...
  If not defined, the platform must implement ``tfm_hal_its_max_asset_size()``.
- ``ITS_NUM_ASSETS`` - Defines the maximum number of assets to be stored in the
  ...
  dynamic memory allocation. If not defined, the platform must implement
  ``tfm_hal_its_max_num_assets()``.
